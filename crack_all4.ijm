...
function crack(input, output, filename, rowNum, n){
	...
	runMacro("FeatureExtraction_findCracksTissue.ijm",n);
	...
	saveAs("tiff", output + filename+n+"_all_crack_t");			
	run("Find Edges");//saveAs("tiff", output + filename+"_all_crack");	
